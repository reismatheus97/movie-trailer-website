...
for currentPage in range(1, totalPages):
    # Only in the first page the request goes
    # without "page" query string parameter.
    if currentPage != 1:
        response = requests\
                    .get("https://api.themoviedb.org/3/search/movie?api_key=" +
                         API_KEY +
                         "&query=Star%20Wars&page=" +
                         str(currentPage))
        responseJson = json.loads(response.content)
    ...
    for item in responseJson['results']:
        ...
        movie_title = item['title']\
            ...
            .decode()  # Removing non-ASCII chars.
        movie_popularity = item['popularity']
        poster_image_url = "https://image.tmdb.org/t/p/w300" +\
        ...
        # Add the instance do the movie array.
        starWarsMoviesArray.append(movieInstance)

# Sort the movie array by the key "movie_popularity"
starWarsMoviesArray = sorted(starWarsMoviesArray,
                             key=lambda k: k['movie_popularity'],
                             reverse=True)
